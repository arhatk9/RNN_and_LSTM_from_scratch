...
def initialization_of_adam(parameters) :
    """
...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate = 0.01, beta1 = 0.9, beta2 = 0.999,  epsilon = 1e-8):
    """
    Updating the parameters using Adam
    ...
    L = len(parameters) // 2                 # number of layers in the neural networks
    v_correct = {}                         # Initializing first moment estimate, python dictionary
    s_correct = {}                         # Initializing second moment estimate, python dictionary
    ...
    for i in range(L):
        ...
        # Compute bias-corrected second raw moment estimate. Inputs: "s, beta2, t". Output: "s_correct".
        s_correct["dW" + str(i+1)] = s["dW" + str(i+1)] / (1 - beta2 ** t)
        s_correct["db" + str(i+1)] = s["db" + str(i+1)] / (1 - beta2 ** t)
        

        # Update parameters. Inputs: "parameters, learning_rate, v_correct, s_correct, epsilon". Output: "parameters".
        parameters["W" + str(i+1)] = parameters["W" + str(i+1)] - learning_rate * v_correct["dW" + str(i+1)] / np.sqrt(s_correct["dW" + str(i+1)] + epsilon)
        parameters["b" + str(i+1)] = parameters["b" + str(i+1)] - learning_rate * v_correct["db" + str(i+1)] / np.sqrt(s_correct["db" + str(i+1)] + epsilon)
